...
//                    $(this).parent().remove();
                             ...
                             $("#tipDiv").find("p").html("存在使用该颜色的模版，无法删除！<br>Can't delete the Color which has been used in model No.!");
                             $("#tipDiv").dialog("open");
                         }
                     },
                     error: function(e) {
...
$.ajax({
    ...
    success: function (res) {
        var n = 0;
        for(var i = res.colour.length - 1;i >= 0;i--){
            n++;
...
//添加功能
 $("#addBtn").click(function(){
    ...
    { $("#enDialog").dialog( "open" );}
        ...
        $("#tipDiv").find("p").html('颜色输入请不要含有英文 , 逗号！<br>The input of Color should not contain comma: " , "');
        $("#tipDiv").dialog("open");
    }else {
        //测试
        //n=$(".t2").length+1;
