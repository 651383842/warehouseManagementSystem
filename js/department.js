/**
 * Created by jessicababy on 2017/5/18.
 */
var click=null;
//打开页面展示信息
$.ajax({
    ...
    success: function (res) {
        var n = 0;
        for(var i = res.department.length - 1;i >= 0;i--){
            n++;
            $("#tab").append("<tr class='t2'>" +
                "<td>"+n+"</td>" +
                "<td>"+res.department[i]+"</td>" +
...
$("#addBtn").click(function(){
    ...
    { $("#enDialog").dialog( "open" );}
        ...
        $("#tipDiv").find("p").html('系列输入请不要含有英文 , 逗号！<br>The input of Department should not contain comma: " , "');
        $("#tipDiv").dialog("open");
    }else{
        ////测试
        //n=$(".t2").length+1;
    ...
    buttons: [
        {
            ...
            click: function() {
                $.ajax({type:"POST",
                    ...
                    dataType: "json",
                    data:{ "department":click.parent().find("td").eq(1).html()},
                    success:function(res){
...
//                        alert("操作成功success");
                            ...
                            $("#tipDiv").find("p").html("存在使用该系列的模版，无法删除！<br>Can't delete the Department which has been used in model No.!");
                            $("#tipDiv").dialog("open");
                        }
                        },
                    error: function(e) {
