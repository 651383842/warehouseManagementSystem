...
$.ajax({
    ...
    success: function (res) {
        var n = 0;
        for(var i = res.fitting.length - 1;i >= 0;i--){
            n++;
...
$("#addBtn").click(function(){
    ...
    { $("#enDialog").dialog( "open" );}
        ...
        $("#tipDiv").find("p").html('型号输入请不要含有英文 , 逗号！<br>The input of Fitting should not contain comma: " , "');
        $("#tipDiv").dialog("open");
    }else {
        ////测试
        //n=$(".t2").length+1;
        ...
        $.ajax({
            type: "POST",
            url: "/Manage/addFitting",
            dataType: "json",
            data: {
                "fitting":result+"/"+result2
            },
            success:function (res){
    ...
    buttons: [
        {
            ...
            click: function() {
                $.ajax({type:"POST",
                    url: "/Manage/deleteFitting",
                    dataType: "json",
                    data:{ "fitting":click.parent().find("td").eq(1).html()},
                    success:function(res){
...
//                        alert("操作成功success");
                            ...
                            $("#tipDiv").find("p").html("存在使用该型号的模版，无法删除！<br>Can't delete the Fitting which has been used in model No.!");
                            $("#tipDiv").dialog("open");
                        }},
                    error: function(e) {
                        console.log("styleError:"+e);
