    ...
    buttons: [
        {
            ...
            click: function() {
                $.ajax({type:"POST",
                        url: "/Manage/deleteSize",
                        dataType: "json",
                        data:{ "size":click.parent().find("td").eq(1).html()},
...
//                        alert("操作成功success");
                                ...
                                $("#tipDiv").find("p").html("存在使用该尺码的模版，无法删除！<br>Can't delete the Size which has been used in model No.!");
                                $("#tipDiv").dialog("open");
                            }},
                        error: function(e) {
...
$.ajax({
    ...
    success: function (res) {
        var n = 0;
        for(var i = res.size.length - 1;i >= 0;i--){
            n++;
...
$("#addBtn").click(function(){
    ...
    { $("#enDialog").dialog( "open" );}
        ...
        $("#tipDiv").find("p").html('尺码输入请不要含有英文 , 逗号！<br>The input of Size should not contain comma: " , "');
        $("#tipDiv").dialog("open");
    }else {
        ////测试
        //n=$(".t2").length+1;
