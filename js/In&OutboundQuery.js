			...
			if(res.UserInfomation.role != 0){
                ...
                try{
                    ...
                    if(param == "outbound"){
                        ...
                        $("#query").click();
                        /*var pacID = JSON.parse(localStorage.getItem("outboundFailurePacID"));
                        var modelNoList = JSON.parse(localStorage.getItem("outboundFailureModelNo"));
                        var num = 1;
                            ...
                            $.ajax({
                                ...
                                success: function (res) {
                                    ...
                                    for (var i = 0; i < res.wares.length; i++) {
                                        for (var j = 0; j < modelNoList.length; j++) {
                                            if(res.wares[i].moduleNum == modelNoList[j][1]) {
                                                $("#tab").append("<tr class='t1'> " +
                                                    "<td></td> " +
                                                    "<td>" + res.wares.UserId + "</td> " +
                                                    "<td>" + res.wares.packageID + "</td> " +
                                                    "<td>" + res.wares.moduleNum + "</td> " +
                                                    "<td>" + res.wares.department + "</td> " +
                                                    "<td>" + res.wares.brand + "</td> " +
                                                    "<td>" + res.wares.fitting + "</td> " +
                                                    "<td>" + res.wares.provider + "</td> " +
                                                    "<td>" + res.wares.size + "</td> " +
                                                    "<td>" + res.wares.colour + "</td> " +
                                                    "<td>" + res.wares.positionInfo.substring(0, 2) + "-" + res.wares.positionInfo.substring(2, 5) + "</td> " +
                                                    "<td>" + res.wares.time + "</td> " +
                                                    "<td>" + res.wares.amount + "</td> " +
                                                    "<td></td> " +
                                                    "<td></td> " +
                                                    "</tr>");
                                            }
                                    ...
                                    num++;
                                    if(num == pacID.length){
                                        $(".t1").first().find("td").eq(13).html("<td><img class='print' src='./img/print.png' alt='打印-Print'></td>");
                                        $("#t1").find("td").eq(0).remove();
                                        $("#t1").find("td").eq(1).html("序号-No");
                                        $("#t1").find("td").eq(11).html("出库数量-Outbound Num.");
                                        $("#t1").find("td").eq(13).remove();
                                        $("#t1").find("td").eq(12).remove();
                                        $("#t1").find("td").eq(10).remove();
                                        $(".print").parent().click(function () {
                                            var tabClone = $("#tab").clone();
                                            tabClone.find(".t1").each(function () {
                                                $(this).find("td").eq(0).remove();
                                                $("#t1").find("td").eq(13).remove();
                                                $("#t1").find("td").eq(12).remove();
                                                $("#t1").find("td").eq(10).remove();
                                            });
                                            localStorage.setItem("out", user);
                                            localStorage.setItem("operationTime", $("#tab").find(".t1").first().find("td").eq(11).html());
                ...
                success: function (res) {
                    ...
                    if (res.wares.length == 0) {
                        ...
                        $(".print").parent().click(function () {
                            var tabClone = $("#tab").clone();
                            var packetID = $(this).parent().find("td").eq(2).html();
                            var shelf = ($(this).parent().find("td").eq(10).html()).substring(0, 2);
                            var position = ($(this).parent().find("td").eq(10).html()).substring(3, 6);
                            var total = $(this).parent().find("td").eq(12).html();
                            ...
                            tabClone.find("#t1").find("td").eq(14).remove();
                            tabClone.find("#t1").find("td").eq(13).remove();
