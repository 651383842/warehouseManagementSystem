/**
 * Created by jessicababy on 2017/5/3.
 */
var click=null;
$("#delDialog").dialog({
    autoOpen: false,
    ...
    buttons: [
        {
            ...
            click: function() {
                $.ajax({type:"POST",
                    url: "/Manage/deleteProvider",
                    dataType: "json",
                    data:{ "provider":click.parent().find("td").eq(1).html()},
                    success:function(res){
                        if(res.backInfo=="delete success") {
                            location.href = location.href;
                        }else if(res.backInfo=="cant"){
                            $("#tipDiv").find("p").html("存在使用该供应商的模版，无法删除！<br>Can't delete the Supplier which has been used in model No.!");
                            $("#tipDiv").dialog("open");
                        }
                    },
                    error: function(e) {
...
$.ajax({
    ...
    success: function (res) {
        var n = 0;
        for(var i = res.provider.length - 1;i >= 0;i--){
            n++;
            $("#tab").append("<tr class='t2'>" +
                "<td>"+n+"</td>" +
                "<td>"+res.provider[i]+"</td>" +
...
$("#btn1").click(function(){
    var n;
    var str=$("#supplierCh").val();
    var result=str.replace(/(^\s+)|(\s+$)/g,"");//去掉前后空格
    result=result.replace(/(^\s{2,})|(\s{2,}$)|(\s{2,})/g," ");
    //alert(result);
    var str2=$("#supplier").val();
    ...
    { $("#enDialog").dialog( "open" );}
        ...
        $("#tipDiv").find("p").html('供应商输入请不要含有英文 , 逗号！<br>The input of Supplier should not contain comma: " , "');
        $("#tipDiv").dialog("open");
    }else {
        //测试
        //n=$(".t2").length+1;
        ...
        $.ajax({
            type: "POST",
            url: "/Manage/addProviderInfo",
            dataType: "json",
            data: {
                "provider":result+"/"+result2
            },
            success:function (res){
...
$("#supplier").keydown(function(event) {
